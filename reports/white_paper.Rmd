...
The map below overlays portfolio properties on ZCTA classifications. Use the layer controls in the top-right to toggle ZCTA boundaries and property markers on/off. Click on any property or ZCTA for detailed metrics.
...
if (nrow(zcta_classifications_full) > 0) {
  ...
  # Create map - properties added AFTER polygons so they render on top
  leaflet(width = "100%", height = 600) %>%
    ...
    addLayersControl(
      overlayGroups = c("ZCTA", "Properties"),
